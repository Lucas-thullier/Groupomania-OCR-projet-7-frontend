...
const SingleUserSearchPreview = ({ singleSearchResult }) => {
  const [isAlreadyFriend, setIsAlreadyFriend] = useState(
    singleSearchResult.isAlreadyFriend
  )
  ...
  return (
    <div
      className="singleUserSearchPreview"
    ...
    >
      ...
      <div
        ...
        onClick={handleClickOnContextualButton}
      >
        <span className="fa-layers fa-fw" id="userStatut">
          <FontAwesomeIcon
            icon={faUserFriends}
            className="friendshipLogo"
          />
          <FontAwesomeIcon
            icon={isAlreadyFriend ? faCheck : faPlus}
            transform="shrink-4"
            className="friendshipCheck"
          />
        </span>
      </div>
    </div>
